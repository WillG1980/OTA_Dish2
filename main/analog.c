...
// - Preserves legacy line: "Current ADC reading: <int>" at the chosen log cadence
// - Uses ADC calibration (line or curve fitting) when available
//
// Notes:
// • Set VSUPPLY_MV to 3300 or 5000 depending on your divider feed.
// • Set THERM_ON_TOP true if Thermistor→Vsupply, Rk→GND (counts rise with temp),
...
static inline bool program_running(void) {
    // Treat non-empty Program string as "running"; adjust if you use a sentinel like "None".
...
static esp_err_t init_adc_oneshot(void) {
    ...
    // Calibration (prefer line fitting where available; fallback to curve if present)
...
#if ADC_CALI_SCHEME_LINE_FITTING_SUPPORTED
    {
        ...
        if (adc_cali_create_scheme_line_fitting(&cal_cfg, &s_cali) == ESP_OK) {
            s_cal_ok = true; calibrated = true; _LOG_I("ADC calibration: line fitting enabled");
        }
    }
#endif
    ...
    if (!calibrated) {
        ...
        if (adc_cali_create_scheme_curve_fitting(&cal_cfg, &s_cali) == ESP_OK) {
            s_cal_ok = true; calibrated = true; _LOG_I("ADC calibration: curve fitting enabled");
        }
    }
#endif
    if (!calibrated) { s_cal_ok = false; _LOG_W("ADC calibration not supported; using raw->mV fallback"); }
...
static void collect_full_sample(SampleStats *out) {
    ...
    for (int i = 0; i < OVERSAMPLE_N; ++i) {
        ...
        if (er != ESP_OK) { _LOG_W("adc_oneshot_read error=%d", (int)er); r = 0; }
        buf[i] = r;
    ...
    // EWMA based on mean
    if (isnan(s_ewma)) s_ewma = mean_raw_f; else s_ewma = (1.0f - EWMA_ALPHA)*s_ewma + EWMA_ALPHA*mean_raw_f;
...
static void temp_sampler_task(void *arg) {
    (void)arg;
    if (init_adc_oneshot() != ESP_OK) { _LOG_E("ADC init failed; exiting sampler task"); vTaskDelete(NULL); return; }
    s_running = true; s_ewma = NAN;
    ...
    while (s_running) {
        ...
        if (!program_running()) {
            // idle: do not collect or log; sleep a bit, clear EWMA so we don't carry stale state
            if (!isnan(s_ewma)) s_ewma = NAN;
            ...
            if ((now - last_log_ms) >= (_LOG_FREQ_ * 1000)) {
                ...
                _LOG_I(
                    "ADC_SAMPLE {raw_inst:%d,mv_inst:%d,raw_mean:%d,mv_mean:%d,raw_min:%d,raw_max:%d,raw_std:%.1f,ewma:%.1f,atten_db:%d,bit:%d,vs_mv:%.0f,top:%d,Rk_ohm:%.0f,Rth_ohm:%.0f,tempC:%.2f,tempF:%.2f,os_n:%d}",
                    st.raw_inst, st.mv_inst, st.raw_mean, st.mv_mean, st.raw_min, st.raw_max, (double)st.raw_std,
                    (double)st.ewma, (int)ANALOG_ADC_ATTEN, (int)ANALOG_BITWIDTH, (double)VSUPPLY_MV,
            ...
            const uint32_t wait_ms = (elapsed >= SAMPLE_PERIOD_MS) ? 1 : (SAMPLE_PERIOD_MS - elapsed);
