...
static void analog_task_(void *arg) {
  ...
  while (g.running) {
    ...
    if (adc_oneshot_read(g.adc_handle, g.cfg.channel, &raw) != ESP_OK) {
      _LOG_I("adc_oneshot_read failed");
      raw = 0;
    }
...
bool analog_init(const analog_config_t *cfg) {
  ...
  if (!g.voltage_v || !g.unknown_r_ohm) {
    _LOG_I("malloc failed for ring buffers");
    return false;
  }
  ring_reset_();
...
bool analog_start(void) {
  if (g.running) {
    _LOG_I("analog_start: already running");
    return true;
  }
  g.running = true;
  ...
  if (ok != pdPASS) {
    g.running = false;
    _LOG_I("failed to create analog_task");
    return false;
  }
...
bool _start_temp_monitor(void) {
  if (!analog_init_defaults()) {
    _LOG_I("_start_temp_monitor: init failed");
    return false;
  }
  if (!analog_start()) {
    _LOG_I("_start_temp_monitor: start failed");
    return false;
  }
  _LOG_I("temperature monitor started (1 Hz, weighted 60s avg, log every 30s)");
  return true;
}
